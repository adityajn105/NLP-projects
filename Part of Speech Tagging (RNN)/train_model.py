...
POS_SIZE = ner.POS.nunique() + 1
print("Output Size", POS_SIZE, "Total Words", VOCAB_SIZE)

#create idx to output, output to idx dictionaries
POS_DICT, REVERSE_POS_DICT = {}, {}
POS_DICT['<pad>'], REVERSE_POS_DICT[0], idx  = 0, '<pad>', 1
for pos in ner.POS.unique():
    POS_DICT[pos] = idx
    REVERSE_POS_DICT[idx] = pos
    idx+=1

#apply indexing to word and output
ner['Word'] = ner['Word'].apply( lambda x: tokens.get( x.lower(), UNK_IDX) )
ner['POS'] = ner['POS'].apply(lambda x: POS_DICT[x])

#prepare input sequence and target sequence
sentences = ner.groupby('Sentence #').Word.apply(lambda x: np.array(x)).values
targets = ner.groupby('Sentence #').POS.apply(lambda x: np.array(x)).values
print("all sentences", sentences.shape,"all outputs",targets.shape)

#We are covering 98% of sentences, take percentile to confirm
MAX_SEQ_LEN = 40
...
targets_final = np.zeros( ( *targets.shape, POS_SIZE) )
for i in range(TOTAL_SIZE):
  for j in range(MAX_SEQ_LEN):
    targets_final[ i, j, targets[i,j] ] = 1
print("final target shape", targets_final.shape)
...
for epoch in range(1,101):
  ...
  print(f" Epoch {epoch:3.0f} - {(time.time()-t0):.0f}s - loss: {loss/i:.4f} - accuracy: {acc/i:.4f} - val_loss: {metrics_val[0]:.4f} - val_accuracy: {metrics_val[1]:.4f}")
save_model( model, "pos_tagging_model.h5" )
...
ax[0].set_title("POS Model Training and Validation Loss curves")
...
ax[1].set_title("POS Model Training and Validation Accuracy Curves")


#a function to generate tag of each sentence
def getTags(sentence):
  ...
  ans = [ np.argmax(pos) for pos in ans ][:ln]
  return " ".join([ REVERSE_POS_DICT[pos] for pos in ans ])
